...
class TestComponent {
  ...
  readonly fieldA = new FormControl('ngxTrimDirective Field A   ');
...
describe('NgxTrimDirective', () => {
  ...
  it('should trim the value of input2 initially', fakeAsync(async () => {
    ...
    expect(el2.value).toBe('ngxTrimDirective Field A');
    expect(component.fieldA.value).toBe('ngxTrimDirective Field A');
  }));
  ...
  it('should not update the value of fieldB on input', fakeAsync(async () => {
    tick();
    await fixture.whenStable();
